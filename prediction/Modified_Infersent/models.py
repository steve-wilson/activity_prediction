...
#        print(sent)
#        print(sent.shape)
#        print("sent_len_sorted")
#        print(sent_len_sorted)
#        print(sent_len_sorted.shape)
#        print("idx_sort")
#        print(idx_sort)
...
#        print ("entering enc_lstm")
#        sys.stdout.flush()
#        print(sent_packed)
#        print(sent_packed.data,sent_packed.batch_sizes)
#        print(sent_packed.data.shape)
#        print(self.enc_lstm)
#        sys.stdout.flush()
        sent_output = self.enc_lstm(sent_packed)[0]  # seqlen x batch x 2*nhid
#        print ("done with enc_lstm")
#        sys.stdout.flush()
...
"""
Main module for Activity Prediction
"""
...
class APNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        # classifier (submodule 4)
        self.nonlinear_fc = config['nonlinear_fc']
        self.fc_dim = config['fc_dim']
        self.n_classes = config['n_classes']
        self.tweet_enc_dim = config['tweet_enc_dim']
        self.profile_enc_dim = config['profile_enc_dim']
        self.user_enc_dim = config['user_enc_dim']
        self.tweet_encoder_type = config['tweet_encoder_type']
        self.profile_encoder_type = config['profile_encoder_type']
        self.user_encoder_type = config['user_encoder_type']
        self.dpout_fc = config['dpout_fc']
        self.values_dim = config['values_dim'] if config['use_values'] else 0
        self.no_profiles = config['no_profiles']
        self.no_tweets = config['no_tweets']

        # tweet_encoder (submodule 1)
        self.tweet_input_dim = 0
        if not self.no_tweets:
            tweet_config = {'bsize':config['bsize'], 'enc_lstm_dim':config['tweet_enc_dim'],\
                             'word_emb_dim':config['word_emb_dim'], 'pool_type':config['pool_type'],\
                             'dpout_model':config['dpout_model']}
            self.tweet_encoder = eval(self.tweet_encoder_type)(tweet_config)

            self.tweet_input_dim = self.tweet_enc_dim
            self.tweet_input_dim = 2*self.tweet_input_dim if self.tweet_encoder_type in \
                            ["ConvNetEncoder", "InnerAttentionMILAEncoder", "InferSent", "BGRUlastEncoder"] else self.tweet_input_dim
            self.tweet_input_dim = self.tweet_input_dim/2 if self.tweet_encoder_type == "LSTMEncoder" \
                                            else self.tweet_input_dim

        # profile encoder (submodule 3)
        self.profile_input_dim = 0
        if not self.no_profiles:
            profile_config = {'bsize':config['bsize'], 'enc_lstm_dim':config['profile_enc_dim'],\
                             'word_emb_dim':config['word_emb_dim'], 'pool_type':config['pool_type'],\
                             'dpout_model':config['dpout_model']}
            self.profile_encoder = eval(self.profile_encoder_type)(profile_config)

            self.profile_input_dim = self.profile_enc_dim
            self.profile_input_dim = 2*self.profile_input_dim if self.profile_encoder_type in \
                            ["ConvNetEncoder", "InnerAttentionMILAEncoder", "InferSent", "BGRUlastEncoder"] else self.profile_input_dim
            self.profile_input_dim = self.profile_input_dim/2 if self.profile_encoder_type == "LSTMEncoder" \
                                            else self.profile_input_dim

        # user encoder (submodule 2)
        # takes all tweets as input and produces a vector
        self.user_input_dim = 0
        if not self.no_tweets:
            user_config = {'bsize':config['bsize'], 'enc_lstm_dim':config['user_enc_dim'],\
                             'word_emb_dim':self.tweet_input_dim, 'pool_type':config['pool_type'],\
                             'dpout_model':config['dpout_model']}
            self.user_encoder = eval(self.user_encoder_type)(user_config)
            self.user_input_dim = self.user_enc_dim
            self.user_input_dim = 2*self.user_input_dim if self.user_encoder_type in \
                            ["ConvNetEncoder", "InnerAttentionMILAEncoder", "InferSent", "BGRUlastEncoder"] else self.user_input_dim
            self.user_input_dim = self.user_input_dim/2 if self.user_encoder_type == "LSTMEncoder" \
                                            else self.user_input_dim

        # this is the classifier's input dimension
        # will be concatenation of user embedding and profile embedding
        self.inputdim = self.user_input_dim + self.profile_input_dim + self.values_dim
...
#                nn.Linear(self.fc_dim, self.fc_dim),
    ...
    def forward(self, tweets, profile, values=torch.Tensor().cuda()):

        if not self.no_tweets:
            tweets_emb_list = [self.tweet_encoder(zip_tweet_length) for zip_tweet_length in zip(tweets[0],tweets[1])]
            tweets_emb_lengths = np.array([x.shape[0] for x in tweets_emb_list])

            # handle padding since users will have different numbers of tweets
            max_tweets = max([t.shape[0] for t in tweets_emb_list])
            tweets_emb_list = [nn.functional.pad(t, [0,0,0,max_tweets-t.shape[0]]) for t in tweets_emb_list]

            tweets_emb = torch.stack(tweets_emb_list,1)
            user_emb_tweets_part = None
            if False:
                user_emb_tweets_part = self.user_encoder((tweets_emb,tweets_emb_lengths))
            # TESTING
        #----
            else:
    #            print(tweets_emb.shape)
                user_emb_tweets_part = torch.mean(tweets_emb,0)
    #            print(user_emb_tweets_part.shape)
        #----
        else:
            user_emb_tweets_part = torch.Tensor().cuda()
        prof_emb = None
        if not self.no_profiles:
            prof_emb = self.profile_encoder(profile)
        else:
            prof_emb = torch.Tensor().cuda()
        user_emb = torch.cat((user_emb_tweets_part, prof_emb, values),1)

        output = self.classifier(user_emb)
        return output

#    def encode_tweet(self, tweet):
#        return self.tweet_encoder(tweet)

#    def encode_user(self, tweets_emb, profile, values=torch.Tensor()):
#        return torch.cat( (self.user_encoder(tweets_emb), self.profile_encoder(profile), values), 1)

#    def encode_profile(self, profile, values=torch.Tensor()):
#        return torch.cat((self.profile_encoder(profile), values),1)

    def get_all_weights(self):
        outstr = ""
        outstr += str(self.tweet_encoder.enc_lstm.weight.data.cpu()) + '\n'
        outstr += str(self.profile_encoder.enc_lstm.weight.data.cpu()) + '\n'
        outstr += str(self.user_encoder.convnet1.weight.data.cpu()) + '\n'
        outstr += str(self.user_encoder.convnet2.weight.data.cpu()) + '\n'
        outstr += str(self.user_encoder.convnet3.weight.data.cpu()) + '\n'
        outstr += str(self.user_encoder.convnet4.weight.data.cpu()) + '\n'
        outstr += str(self.classifier.weight.data.cpu()) + '\n'
        return outstr
