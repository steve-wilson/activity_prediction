...
#        print(str(type(self.enc_lstm.bias_hh_l0.data.cuda())))
...
class NLINet(nn.Module):
    def __init__(self, config):
        ...
        self.MTL_index = config['MTL_index']
        ...
        for i in self.MTL_index:
            exec('self.model' + str(i) + '=nn.Sequential(nn.Linear(8192,self.n_classes), nn.Softmax(), ).cuda()' )
        

        #if 2 in self.MTL_index:
        #    if 'e' in self.MTL_index:
        #        self.model2 = nn.Sequential(
        #            nn.Linear(8192, 3),
        #        ).cuda()
        #        self.loss_fn2 = nn.CrossEntropyLoss().cuda()

        #if 'kl' in self.MTL_index:+
        if False:
            self.loss_fn = nn.KLDivLoss().cuda()
        #else:
        if True:
            self.loss_fn = nn.MSELoss().cuda()
        #self.loss_fn.size_average = False

    def forward(self, s, sid):
        # s1 : (s1, s1_len)
#        print("S[0]:"+str(type(s[0])))
#        print("S[1]:"+str(type(s[1])))
#        print("Sid:"+str(sid))
#        print(str(len(sid)))
        u = self.encoder(s)
        u2 = self.model0(u)
#        print("U size:"+str(u.size()))
#        u_flat = u.view(u.numel())
#        print u_flat
        for i,row in enumerate(u2):
#            print("Row size:"+str(row.shape))
#            print(str(sid[i]))
#            sys.stdout.flush()
            with open("/scratch/mihalcea_fluxg/steverw/acts900b/"+str(sid[i])+".vec",'w') as out:
                #out.write(str(sid)+'\t'+str(u.cpu().numpy())+'\n')
                out.write(' '.join([str(float(num.double())) for num in row])+'\n')
        #torch.save(u, "/scratch/mihalcea_fluxg/steverw/acts/"+str(sid[0])+".tsr")
        return []
